...
from sport_center.domain import Username, Password, Price, Description, Name
...
@patch('requests.post', side_effect=[mock_response_dict(200, {'key': '2aaec33e8100720d8281385d11502cc7631923cf'})])
@patch('requests.get', side_effect=[mock_response_dict(200)])
@patch('builtins.input', side_effect=['1', 'simohassssan', 'Mohammed123', '0', '0'])
@patch('builtins.print')
...
def test_app_sign_in_resists_wrong_password(mocked_print, mocked_input, mocked_requests_get, mocked_requests_post):
    ...
    mocked_requests_post.assert_called()
    mocked_requests_get.assert_called()
    mocked_print.assert_any_call('*** Welcome to our SPORT-CENTER App ***')


@patch('requests.post', side_effect=[mock_response_dict(400)])
@patch('builtins.input', side_effect=['1', 'userdontexist', 'userdontexist'])
@patch('builtins.print')
...
@patch('requests.post', side_effect=[mock_response_dict(400)])
@patch('builtins.input', side_effect=['2', 'simohassan', 'email@email.com', 'Mohammed123'])
@patch('builtins.print')
...
@patch('builtins.input', side_effect=['1', 'ssimohassan', 'Mohammed123', '0', '0'])
@patch('builtins.print')
def test_app_sport_center(mocked_print, mocked_input, mocked_requests_get, mocked_requests_post):
    with patch('builtins.open', mock_open()):
        App().run()
    mocked_print.assert_any_call('*** SIGN IN ***')
    ...
    mocked_requests_get.assert_called_once_with(url='http://localhost:8000/api/v1/sport-center/', headers={
        'Authorization': 'Token 2aaec33e8100720d8281385d11502cc7631923cf'})
    mocked_input.assert_called()
